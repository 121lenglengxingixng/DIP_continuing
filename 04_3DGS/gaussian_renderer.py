...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        exponent = -0.5 * (dx.unsqueeze(-2) @ inv_covs.unsqueeze(1).unsqueeze(2) @ dx.unsqueeze(-1)).squeeze(-1).squeeze(-1)  # (N, H, W)

        # Gaussian values
        normalization = 1.0 / (2.0 * np.pi * torch.sqrt(det_covs)).unsqueeze(1).unsqueeze(2)  # (N, 1, 1)
    ...
    def forward(
        ...
        T = torch.cat([torch.ones(1, H, W, device=T.device), T[:-1]], dim=0)  # Shift by one for proper transmittance
