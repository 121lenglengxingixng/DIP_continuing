...
class GaussianModel(nn.Module):
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        R = self._compute_rotation_matrices()  # Shape: (N, 3, 3)
        
        # Convert scales from log space and create diagonal matrices
        scales = torch.exp(self.scales)  # Shape: (N, 3)
        S = torch.diag_embed(scales)     # Shape: (N, 3, 3)
        
        # Compute covariance: Covs3d = R * S * R^T
        Covs3d = torch.bmm(R, torch.bmm(S, R.transpose(1, 2)))  # Shape: (N, 3, 3)
